...
class Gridworld5x5:
    ...
    def transitions(
        ...
        converted_action = actions_to_dxdy(action)
        ...
        if state == self.A:
            ...
            next_state = (state[0] + converted_action[0], state[1] + converted_action[1])
            reward = 0
...
class JacksCarRental:
    ...
    def _open_to_close(self, loc_idx: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for start in range(probs.shape[0]):
            ...
            probability_rent = 0
            for i in range(0, self.max_cars_start+1):

                probability_rent = self.rent[loc_idx].pmf(i)
                avg_rent += probability_rent * i

            if avg_rent >= start:
                rewards[start] = self.rent_reward * start
            else:
            ...
            for end in range(probs.shape[1]):
                ...
                # Be sure to consider the case where business is lost (i.e.+-)
                for i in range(min_rent, start + 1):
                    car_returns = i - min_rent
                    prob += self.rent[loc_idx].pmf(i) * self.return_[loc_idx].pmf(car_returns)
            ...
            for locB in range(self.max_cars_end + 1):
                for ia, action in enumerate(self.action_space):
                    ...
                    if not self._valid_action((locA, locB), action):
                        ...
                        self.r[locA, locB, ia] = day_rewards_A[locA+ia] + day_rewards_B[locB+ia] - self._calculate_cost((locA, locB), action)
